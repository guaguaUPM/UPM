...
subroutine resolver_gauss(A,B,X,N)
    ...
    real*8, allocatable :: A_PIVOT_IN(:,:), B_PIVOT_IN(:)
    real*8, allocatable :: A_PIVOT_OUT(:,:), B_PIVOT_OUT(:)
    integer, allocatable :: PIVOTE(:,:)
    allocate(A_PIVOT_IN(N,N))
    allocate(B_PIVOT_IN(N))
    allocate(A_PIVOT_OUT(N,N))
    allocate(B_PIVOT_OUT(N))
    allocate(PIVOTE(N,N))

    call pivotar(A, A_PIVOT_IN, B, B_PIVOT_IN, PIVOTE, N)
    call gauss_triangular(A_PIVOT_IN, A_PIVOT_OUT, B_PIVOT_IN, B_PIVOT_OUT, N)
    call gauss_sustituir(A_PIVOT_OUT, B_PIVOT_IN, X, N, .TRUE.)
    X = matmul(PIVOTE, X)
